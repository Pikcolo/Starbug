...
[ Tier 1: Text Normalization & Typo Correction ]
   - คลีน Unicode และแก้สระ/วรรณยุกต์ซ้ำซ้อนด้วย PyThaiNLP (newmm engine)
   - ตัดตัวอักษรลากเสียงแชท (เช่น 'เข้มมมม' ➔ 'เข้ม', 'อร่อยยยย' ➔ 'อร่อย')
   - คลังคำแก้คำสะกดผิด 150+ คำ (กาเเฟ ➔ กาแฟ, ชาเขีบว ➔ ชาเขียว, เวนตี้ ➔ Venti)
         │
         ▼
[ Tier 2: Entity Extraction ]
   ...
   - สกัดหมวดหมู่ (frappuccino, bakery, tea, coffee) และรสชาติ (sweet, coffee_strong)
         │
         ▼
[ Tier 3: Longest-Substring & RapidFuzz Priority Matcher ]
   ...
   - คำนวณ RapidFuzz Token Set Ratio (Threshold >= 70%) จับคู่ชื่อเมนูแม้สะกดเพี้ยน
         │
         ▼
[ Tier 4: Neural BERT Classifier (paraphrase-multilingual-MiniLM-L12-v2) ]
   - แปลงประโยคเป็น Dense Semantic Vector 384 มิติ ด้วย Pretrained Neural Transformer
   - คำนวณ Cosine Similarity เทียบกับ Intent Vector Space (Threshold > 0.45)
   - ตัดสินใจเจตนาที่แท้จริง พร้อมค่า Confidence Score (0.0 - 1.0)
```

### 🔹 ทำไมต้องใช้ Real Neural BERT Transformer?
- จัดการ **ภาษาพูด, ภาษาแสลง, และประโยคเปรียบเปรย** ที่ไม่มีคีย์เวิร์ดตายตัว เช่น *"ตาจะปิด ขออะไรเข้มๆ"* หรือ *"ขี้เกียจคิด เอาอะไรก็ได้มาสักแก้ว"*
- วิเคราะห์บริบทความหมายแบบข้ามภาษา (Cross-lingual Semantic Matching) ได้อย่างแม่นยำ

### 🔹 ประสิทธิภาพของ NLP Engine:
- **Accuracy Rate:** **100.00%** (36/36 Benchmark Test Cases)
- **Average Latency:** **~5.3 ms** (เร็วกว่าเกณฑ์มาตรฐาน 1,500 ms ถึงเกือบ 300 เท่า)
...
### 5.2 Interactive 2-Step Ordering & Dynamic Pricing
- **ขั้นตอนที่ 1 (เลือกขนาดแก้ว):** เมื่อผู้ใช้ระบุชื่อเมนูเจาะจงโดยยังไม่ได้ระบุไซส์ บอทจะไม่ตัดยอดทันที แต่จะส่ง **Product Detail Card** พร้อมปุ่มเลือกขนาดแก้วจริง เช่น `[ สั่ง Solo (฿100) ]`, `[ สั่ง Doppio (฿115) ]` หรือ `[ สั่ง Tall ]`, `[ สั่ง Grande ]`, `[ สั่ง Venti ]`
- **ขั้นตอนที่ 2 (ยืนยันและคิดราคา):** เมื่อผู้ใช้กดปุ่มเลือกไซส์ หรือพิมพ์ระบุไซส์ชัดเจน ระบบจะคำนวณราคาของไซส์นั้นจริง และออก **ใบเสร็จยืนยันออเดอร์ (Flex Order Receipt)** พร้อมลิงก์ไปหน้าเว็บชำระเงิน

### 5.3 Low-Budget Smart Handling (< ฿65)
- หากผู้ใช้ระบุงบต่ำกว่าสินค้าที่ถูกที่สุดในสตาร์บัคส์ (เช่น งบ 50 บาท) บอทจะตอบมุกแจ้งเตือนอย่างจริงใจ พร้อมดึง **5 เมนูที่ราคาประหยัดที่สุดในร้าน (฿65 - ฿85)** มาแสดงเรียงตามลำดับ
...
2. **6 Floating Quick Reply Chips:**
   - ปุ่มทางลัดลอยเหนือช่องพิมพ์ข้อความ: ⭐ เมนูแนะนำวันนี้ / ☕ เมนูกาแฟสด / 🥐 เบเกอรี่ & เค้ก / 🏷️ โปรโมชั่นเด็ด / 📖 วิธีการใช้งาน / 🤣 บาริสต้าปากแซ่บ
3. **LINE Flex Message Carousel, Detail, and Receipt Cards:**
...
|---|---|---|---|---|
| **1** | **💬 ทักทาย & ต้อนรับ (Greeting)** | ต้อนรับผู้ใช้ด้วยมุกบาริสต้าภารตะ พร้อม Quick Reply ทางลัด (ไม่รบกวนด้วย Carousel เมนู) | `สวัสดีครับ`, `หวัดดีจ้า`, `hello` | ส่งรูปบาริสต้าดอลลี่ + ข้อความต้อนรับ + Quick Reply ปุ่มทางลัด |
| **2** | **📖 คู่มือช่วยเหลือ (Help Guide)** | สรุป 8 แนวทางการสั่งการระบบบอท พร้อม Quick Reply | `วิธีการใช้งาน`, `ช่วยด้วย ใช้ยังไง`, `คู่มือ` | ข้อความคู่มือ 8 ข้อ + Quick Reply ปุ่มทางลัด |
...
| **7** | **💸 กรองตามงบ (Price Filter)** | กรองเมนูตามงบประมาณ (ถ้างบต่ำกว่า ฿65 จะส่งมุกแซว + 5 เมนูถูกสุด) | `งบไม่เกิน 150 บาท`, `เมนูงบ 50 บาท` | Carousel เมนูตามงบ หรือเมนูประหยัดที่สุด |
| **8** | **😋 ค้นหาตามรสชาติ (Flavor Search)** | กรองตาม Taste Profile (เข้มข้น / สดชื่น) | `บ่ายนี้ง่วงมาก ขอเข้มๆ`, `อยากได้อะไรหวานๆ สดชื่น` | Carousel เมนูกาแฟตาสว่าง หรือ เมนูผลไม้สดชื่น |
...
| **12** | **🔍 ส่องรายละเอียด (Item Detail)** | แสดงรายละเอียด แคลอรี่ ส่วนผสม และตารางราคา | `Iced Americano กี่แคล`, `ขอดูรายละเอียด จาวาชิพ` | การ์ด Flex Detail ขนาดใหญ่บอกสเปกครบ |
| **13** | **🛍️ สั่งซื้อ 2 ขั้นตอน (Interactive Order)** | ส่งการ์ดให้เลือกไซส์ก่อนออกใบเสร็จจริง | `สั่งซื้อเอสเพรสโซ 1 แก้ว`, `สั่งซื้อ โคลด์บรูว์ ขนาด Venti` | การ์ดเลือกขนาด หรือ ใบเสร็จ Flex Receipt |
...
|---|---|---|---|
| **NLP Intent Accuracy** | > 85.00% | **100.00%** (36/36 Test Cases) | ✅ ผ่านเกณฑ์ยอดเยี่ยม |
| **Average Response Latency** | < 1,500 ms | **~5.31 ms** (Peak Latency: 6.20 ms) | ✅ ผ่านเกณฑ์ยอดเยี่ยม |
